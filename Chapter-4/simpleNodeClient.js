...
if (channel !== "") {
    ...
    socket.emit('create_or_join', channel);
}
...
socket.on('remotePeerJoining', function (channel){
    console.log('remotePeerJoining Handle');
...
// Handle 'broadcast_joined' message
socket.on('broadcast_joined', function (msg){
    console.log('broadcast Handle');
    div.insertAdjacentHTML( 'beforeEnd', '<p style="color:red">Time: ' +
        (performance.now() / 1000).toFixed(3) +
...
socket.on('client_response', function (response){
